...
public class BaseballUI {
	...
	public void idDelete() {
		...
		while (true) {
			try {
				System.out.println("아이디를 삭제하시겠습니까? ");
	...
	public void pitcherDelete() {
		ArrayList<UserCharacterVO> list = pitcherSelect();
		if (list != null) {
			if (list.size() != 0) {
	...
	public void hitterDelete() {
		ArrayList<UserCharacterVO> list = hitterSelect();
		if (list != null) {
			if (list.size() != 0) {
				int d = dao.deleteCharacter(presentChar.getUserId(), presentChar.getCharacterId());
				if (d == 1) {
	...
	//투수 캐릭터 목록 출력
	public ArrayList<UserCharacterVO> pitcherSelect() {
		int num = 0;
	...
	//타자 캐릭터 목록 출력
	public ArrayList<UserCharacterVO> hitterSelect() {
		int num = 0;
	...
	public void mainMenu() {
		...
		if (loginId != null && presentChar != null) {
			while (true) {
				...
				case 4: match();  if(presentChar != null) {break;} else {return;}
				case 5: rest(); 				break;
				case 6: quiz();					break;
	...
	public void mainMenuPrint() {
		System.out.println("[ 나만의 선수 키우기 야구편 메인메뉴 ]");
	...
	public void itemInfo() {
		...
		System.out.println("--------------------------------------------------");
		System.out.println(" 아이템 이름 \t" + " 아이템 설명 ");
		for(int i=0;i<list.size();i++) {
	...
	public void match() {
		...
		if (presentChar.getYear() < 5) {
			int bh = presentChar.getHealth();
			presentChar.setActive(5);
			if (presentChar.getHealth() < 75) {
				presentChar.setHealth(presentChar.getHealth() + 25);
			}
			else {
			...
			System.out.println("체력이 " + (presentChar.getHealth() - bh) + "회복 되었습니다.");
			...
			if (m != 0) {
				System.out.println("은퇴선수 목록에 반영되었습니다.");
			}
	...
	public void restMenuPrint() {
		...
		System.out.println("1.	일반 휴식(무료)");
		System.out.println("2.	고급 휴식(300원)");
		System.out.println("0.	뒤로 가기");
		System.out.print("선택>	");
	}
